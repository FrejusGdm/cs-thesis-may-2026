...
"""
T2-zh: Fine-tune Orpheus 3B (Mandarin base) on Adja TTS.

Uses the Mandarin pretrained checkpoint as the starting point instead of English.
Mandarin has 4 lexical tones (realized as F0 contours) — same acoustic mechanism as
Adja/Ewe tonal distinctions. Hypothesis: the zh LM prior improves tone-to-audio mapping.

This is identical to T2_orpheus_finetune.py except the default --base-model is
canopylabs/3b-zh-ft-research_release.

References:
  - Orpheus multilingual variants: https://huggingface.co/canopylabs
  - Tone hypothesis: experiments/asr-tts-getting-right-2026-04-21.md, Track 1C
...
def parse_args():
    p = argparse.ArgumentParser(description="T2-zh: Orpheus 3B zh→Adja TTS")
    p.add_argument("--base-model", default="canopylabs/3b-zh-ft-research_release")
    p.add_argument("--dataset", default="JosueG/adja-tts-orpheus")
    p.add_argument("--output-dir", default="/tmp/orpheus_zh_adja")
    ...
    p.add_argument("--results-prefix", default="T2_orpheus_zh")
    return p.parse_args()
...
def main():
    ...
    for s in (random, ): s.seed(args.seed)
    ...
    def preprocess(ex):
        ...
        if not text: dropped["empty_text"]+=1; return None
        arr = ex.get("audio",{}).get("array")
        if arr is None or len(arr)<10000: dropped["short_audio"]+=1; return None
        try: flat = build_snac_codes(arr, snac_model, orig_sr, "cuda")
        except Exception as e: print(f"  skip: {e}"); dropped["encode_fail"]+=1; return None
        ...
        if len(ids)>args.max_seq_length: dropped["too_long"]+=1; return None
    ...
    trainer = Trainer(
        ...
        args=training_args, callbacks=[EarlyStoppingCallback(early_stopping_patience=args.early_stopping_patience)],
    )
    t0 = time.time()
    stats = trainer.train()
    ...
    # Quick generation (5 sentences)
    model.eval(); snac_model.to("cpu")
    generated, start_tok = [], torch.tensor([[START_OF_HUMAN]], dtype=torch.int64)
    end_toks = torch.tensor([[END_OF_TEXT, END_OF_HUMAN]], dtype=torch.int64)
    generated_dir = output_dir/"generated"; generated_dir.mkdir(exist_ok=True)
    for i in range(min(5, len(test_ds))):
        ...
        try:
            ...
            row = row[:(len(row)//7)*7]
            if not row: raise RuntimeError("no audio frames")
            codes = redistribute_codes(row)
            with torch.inference_mode():
                wav = snac_model.decode(codes).detach().squeeze().cpu().numpy().astype(np.float32)
            p = generated_dir/f"plain_{i:02d}.wav"
    ...
    results = {"experiment": "T2_orpheus_zh", "base_model": args.base_model,
               "training_mode": training_mode, "train_loss": round(train_loss,4),
               "best_eval_loss": round(best_eval,4) if best_eval else None,
               "training_time_min": round(elapsed/60,1), "peak_vram_gb": peak_mem,
               "gpu": torch.cuda.get_device_name(0), "generated": generated, "dropped": dropped}
    ...
    if args.push_to_hub:
        ...
        api.upload_file(path_or_fileobj=metrics_path.read_bytes(),
                        path_in_repo=f"{prefix}/metrics.json", repo_id=args.results_repo, token=token)
        api.upload_folder(folder_path=str(adapter_dir), path_in_repo=f"{prefix}/adapter",
                          repo_id=args.results_repo, token=token)
        api.upload_folder(folder_path=str(generated_dir), path_in_repo=f"{prefix}/generated_audio",
                          repo_id=args.results_repo, token=token)
...
if __name__ == "__main__":
    try: main()
    except Exception: import traceback; traceback.print_exc(); raise
