...
# ///
"""
train_nllb.py — Self-contained NLLB-200 / mBART-50 training script for HF Jobs.

Adapted from:
  <LOCAL_PATH>
Differences from source:
  - Loads data from a HuggingFace Datasets-format repo (columns `fra_Latn`,
    `aj_Latn`, splits train/validation/test) instead of TSVs at nested paths.
  - Defaults wired for `JosueG/adja-fr-mt-acl-paper-private` (paper snapshot).
  - SMOKE=1 env var: caps train to 50 pairs, MAX_EPOCHS=1, EVAL_STEPS=10,
    PATIENCE=1 for end-to-end code-path validation in <10 min on CPU.

Custom training loop (Adafactor + constant-with-warmup), early stopping on
validation chrF, generates test translations with `forced_bos_token_id=aj_Latn`,
uploads `test_metrics.json` to RESULTS_REPO.

Environment variables (set via --env in hf jobs run):
    EXPERIMENT    — path component on RESULTS_REPO; default "paper"
    CONDITION     — path component on RESULTS_REPO; default "R10K_S4K"
    SEED          — e.g. "42"
    MODEL         — e.g. "facebook/nllb-200-distilled-600M" (default)
    DATASET_REPO  — HF Datasets-format repo (default: JosueG/adja-fr-mt-acl-paper-private)
    RESULTS_REPO  — private HF dataset repo for metrics JSONs (REQUIRED)
    SIMILAR_LANG  — lang token to copy embeddings from, or "none"; default: ewe_Latn
    SRC_LANG      — source language token (auto-detected from MODEL if not set)
    SMOKE         — "1" enables smoke-test overrides (50-pair train, 1 epoch, 10-step eval)
"""
...
EXPERIMENT = os.environ.get("EXPERIMENT", "paper")
CONDITION = os.environ.get("CONDITION", "R10K_S4K")
SEED = int(os.environ.get("SEED", "42"))
MODEL_NAME = os.environ.get("MODEL", "facebook/nllb-200-distilled-600M")
DATASET_REPO = os.environ.get("DATASET_REPO", "JosueG/adja-fr-mt-acl-paper-private")
...
if SMOKE:
    ...
    print("[SMOKE] overrides: MAX_EPOCHS=1, EVAL_STEPS=10, PATIENCE=1, train capped to 50 pairs")
...
def download_data():
    """Load HF Datasets-format repo and materialize splits as local TSVs.

    Expects the dataset to expose splits `train`, `validation`, `test` with
    columns `fra_Latn` (source) and `aj_Latn` (target). Writes each split
    as TSV under /tmp/data/ and returns the paths so the downstream code
    (which is TSV-based) is unchanged.
    """
    ...
    print(f"Loading dataset {DATASET_REPO} via load_dataset()...")
    ds = load_dataset(DATASET_REPO, token=token)

    expected_splits = {"train", "validation", "test"}
    missing = expected_splits - set(ds.keys())
    if missing:
        raise RuntimeError(f"Dataset {DATASET_REPO} missing splits: {missing}")
    for s in expected_splits:
        cols = set(ds[s].column_names)
        if not {"fra_Latn", "aj_Latn"}.issubset(cols):
            raise RuntimeError(f"Split {s} missing required columns; got {cols}")

    def write_tsv(split_name: str, out_name: str) -> str:
        out_path = os.path.join(data_dir, out_name)
        rows_written = 0
        with open(out_path, "w", encoding="utf-8") as f:
            for ex in ds[split_name]:
                fra = (ex["fra_Latn"] or "").replace("\t", " ").replace("\n", " ")
                aj = (ex["aj_Latn"] or "").replace("\t", " ").replace("\n", " ")
                if not fra or not aj:
                    continue
                f.write(f"{fra}\t{aj}\n")
                rows_written += 1
    ...
    train_file = write_tsv("train", "train.tsv")
    val_file = write_tsv("validation", "val.tsv")
    test_file = write_tsv("test", "test.tsv")
    structured_file = None  # not provided by HF Datasets-format snapshot
    print("  Note: structured_train.tsv not present in this dataset; subset eval will be skipped")
...
def main():
    ...
    if SMOKE:
        ...
        print(f"  [SMOKE] capped train to {len(train_data)} pairs, val to {len(val_data)} pairs")
