"""
train_nllb_hpc.py — bidirectional NLLB training on Dartmouth SLURM cluster.

Source: copied verbatim from
  <LOCAL_PATH>
  experiments/training/hpc/hf_job_train_hpc.py  (LRAC paper code)
plus three small additions, all flag-gated with defaults that preserve the
original fr→aj behavior:
  - --src-lang / --tgt-lang  override the model-name-derived lang tokens,
    enabling aj→fr without touching training logic.
  - --new-lang               which custom token init_lang_token registers +
    seeds from --similar-lang. Default aj_Latn. Lets aj→fr still init aj_Latn
    from ewe_Latn even though aj_Latn is now the source.
  - --push-to-hub / --hub-repo  after the local checkpoint is saved, push
    it to a public HF model repo at {hub_repo}/{tgt_lang}_from_{src_lang}/seed{N}/
    (skipped by default).

Original docstring follows.

HPC-adapted training script for Dartmouth SLURM cluster.
...
def parse_args():
    ...
    p.add_argument("--test-path", default=None,
                   help="Optional override for the test TSV. "
                        "Default: {data_dir}/shared/test.tsv")
    ...
    # Bidirectional additions (defaults preserve original fr→aj behavior)
    p.add_argument("--src-lang", default=None,
                   help="Source language token (e.g. fra_Latn or aj_Latn). "
                        "Default: derived from model name (NLLB→fra_Latn).")
    p.add_argument("--tgt-lang", default=None,
                   help="Target language token (e.g. aj_Latn or fra_Latn). "
                        "Default: aj_Latn.")
    p.add_argument("--new-lang", default="aj_Latn",
                   help="Custom token to register + init from --similar-lang. "
                        "Default: aj_Latn (regardless of direction).")
    # Public-release additions
    p.add_argument("--push-to-hub", action="store_true", default=False,
                   help="After local checkpoint save, push to a public HF model repo.")
    p.add_argument("--hub-repo", default="JosueG/adja-mt-best",
                   help="Target HF model repo for --push-to-hub.")
    p.add_argument("--hub-subfolder", default=None,
                   help="Override path-in-repo. Default: {tgt_lang}_from_{src_lang}/seed{seed}.")
    return p.parse_args()
...
def evaluate_test(model, tokenizer, test_file, src_lang, tgt_lang,
    ...
    metrics = {
        ...
        # BLEU n-gram precision components (1-gram .. 4-gram, brevity penalty)
        "test_bleu_precisions": list(bleu.precisions),
        "test_bleu_bp": bleu.bp,
        "test_bleu_sys_len": bleu.sys_len,
        "test_bleu_ref_len": bleu.ref_len,
    }

    # ----- Extra distributional metrics + predictions payload -----
    # Wrapped in try/except so any unexpected failure here (sacrebleu API
    # quirks, edge inputs, etc.) still leaves the core BLEU/chrF/TER metrics
    # intact and the job's test_metrics.json gets written normally. Same
    # discipline as the ROUGE-L / perplexity blocks below.
    try:
        sentence_chrf = [
            sacrebleu.sentence_chrf(p, [r]).score for p, r in zip(predictions, references)
        ]
        sentence_chrfpp = [
            sacrebleu.sentence_chrf(p, [r], word_order=2).score for p, r in zip(predictions, references)
        ]
        sentence_bleu = [
            sacrebleu.sentence_bleu(p, [r]).score for p, r in zip(predictions, references)
        ]
        src_word_lens = [len(s.split()) for s in sources]
        ref_word_lens = [len(r.split()) for r in references]
        pred_word_lens = [len(p.split()) for p in predictions]
        len_ratios = [
            (pl / rl) if rl > 0 else 0.0
            for pl, rl in zip(pred_word_lens, ref_word_lens)
        ]
        ...
        def _dist(label, vals):
            ...
            metrics[f"test_{label}_mean"] = float(arr.mean())
            metrics[f"test_{label}_std"] = float(arr.std())
            metrics[f"test_{label}_p10"] = float(np.percentile(arr, 10))
            metrics[f"test_{label}_p50"] = float(np.percentile(arr, 50))
            metrics[f"test_{label}_p90"] = float(np.percentile(arr, 90))

        _dist("sentence_bleu", sentence_bleu)
        _dist("sentence_chrf", sentence_chrf)
        _dist("sentence_chrfpp", sentence_chrfpp)
        _dist("src_word_len", src_word_lens)
        _dist("ref_word_len", ref_word_lens)
        _dist("pred_word_len", pred_word_lens)
        _dist("len_ratio", len_ratios)
        metrics["test_empty_pred_count"] = empty_preds

        pred_vocab = set()
        ref_vocab = set()
        for p in predictions:
            pred_vocab.update(p.split())
        for r in references:
            ref_vocab.update(r.split())
        metrics["test_pred_vocab_size"] = len(pred_vocab)
        metrics["test_ref_vocab_size"] = len(ref_vocab)
        metrics["test_vocab_overlap"] = (
            len(pred_vocab & ref_vocab) / len(ref_vocab) if ref_vocab else 0.0
        )

        # Stash for the caller to dump to test_predictions.jsonl.
        metrics["_predictions_payload"] = [
            {
                "idx": i,
                "src": s,
                "ref": r,
                ...
                "sentence_bleu": sb,
                "sentence_chrf": sc,
                "sentence_chrfpp": scpp,
                "src_word_len": sl,
                "ref_word_len": rl,
                "pred_word_len": pl,
            }
            for i, (s, r, p, sb, sc, scpp, sl, rl, pl) in enumerate(zip(
                sources, references, predictions,
                sentence_bleu, sentence_chrf, sentence_chrfpp,
                src_word_lens, ref_word_lens, pred_word_lens,
            ))
        ]
    except Exception as e:
        print(f"[extra-metrics] skipped due to error: {type(e).__name__}: {e}")
        metrics["test_extra_metrics_error"] = f"{type(e).__name__}: {e}"
...
def main():
    ...
    src_lang = args.src_lang if args.src_lang else src_default
    tgt_lang = args.tgt_lang if args.tgt_lang else "aj_Latn"
    ...
    print(f"  NEW_LANG:      {new_lang}  (init from {similar_lang})")
    print(f"  SIMILAR_LANG:  {similar_lang}")
    print(f"  Push to Hub:   {args.push_to_hub}  ({args.hub_repo})")
    print(f"  LR:            {args.lr}")
    ...
    test_file = args.test_path or os.path.join(args.data_dir, "shared", "test.tsv")
    ...
    # Register the custom Adja token. We always register `new_lang` (default
    # aj_Latn) regardless of which side it appears on — for aj→fr, aj_Latn
    # is the source and still needs the Ewe-init prior; fra_Latn is already
    # in NLLB and needs no init.
    init_lang_token(model, tokenizer, new_lang, similar_lang)
    ...
    # Pull the per-sentence rows out of the metrics dict so test_metrics.json
    # stays compact (only summary stats) — predictions go to a sibling JSONL.
    predictions_payload = test_metrics.pop("_predictions_payload", [])
    with open(result_path, "w", encoding="utf-8") as f:
        json.dump(test_metrics, f, indent=2)
    print(f"\nResults saved to: {result_path}")

    if predictions_payload:
        preds_path = os.path.join(result_dir, "test_predictions.jsonl")
        with open(preds_path, "w", encoding="utf-8") as f:
            for row in predictions_payload:
                f.write(json.dumps(row, ensure_ascii=False) + "\n")
        print(f"Per-sentence predictions saved to: {preds_path} ({len(predictions_payload)} rows)")
    ...
    if args.save_checkpoint:
        ...
        # Drop test_metrics.json next to the weights so the uploaded folder is self-contained.
        ...
        if args.push_to_hub:
            ...
            if not token:
                print("WARNING: --push-to-hub set but HF_TOKEN missing. Skipping push.")
            else:
                ...
                subfolder = args.hub_subfolder or f"{tgt_lang}_from_{src_lang}/seed{args.seed}"
                print(f"Uploading checkpoint to {args.hub_repo}/{subfolder}...")
                api.upload_folder(
                    ...
                    repo_type="model",
                    commit_message=(
                        f"{tgt_lang}_from_{src_lang} seed{args.seed}: "
                        f"chrF={test_metrics['test_chrf']:.2f}"
                    ),
                )
