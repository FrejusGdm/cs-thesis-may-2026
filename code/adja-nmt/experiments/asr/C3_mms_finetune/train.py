...
"""
C3: Fine-tune MMS (facebook/mms-1b-all) on Adja ASR data with CTC.
...
def train(config: dict, data_dir: str, output_dir: str, seed: int, dry_run: bool):
    ...
    # Use FeatureExtractor directly (we have our own CTC vocab, don't need tokenizer)
    feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(model_name)

    # Load with French adapter as starting point — Adja is spoken in
    # Francophone West Africa (Benin/Togo), so French is the closest
    # high-resource language for adapter initialization.
    model = Wav2Vec2ForCTC.from_pretrained(
        model_name,
        target_lang="fra",  # Start from French adapter, not English
    ...
    for epoch in range(max_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # The wav2vec2 model outputs features at ~50Hz from 16kHz input
            # Use the attention_mask to get actual output lengths
            with torch.no_grad():
    ...
    with open(samples_path, "w", encoding="utf-8") as f:
        f.write(f"C3 MMS Fine-Tune -- Decode Samples (best checkpoint)\n")
        f.write(f"{'='*70}\n\n")
...
# ===========================================================================
def main():
    parser = argparse.ArgumentParser(description="C3: Fine-tune MMS on Adja ASR")
    ...
    print(f"C3: MMS Fine-Tune on Adja ASR")
