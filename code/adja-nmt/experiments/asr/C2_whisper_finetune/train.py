...
"""
C2: Fine-tune Whisper on Adja ASR data.

Custom training loop (no HuggingFace Trainer).
Loads data from TSV manifests, trains with AdamW + linear warmup,
evaluates with greedy decoding, early-stops on dev CER.
...
# Dataset
# ===========================================================================
class AdjaWhisperDataset(Dataset):
    """Loads audio + text from a TSV manifest for Whisper fine-tuning."""

    def __init__(self, manifest_path: str, processor, max_audio_length_sec: float = 30.0):
...
def collate_whisper(batch, processor):
    """Collate function: compute mel features and tokenize labels."""
    ...
    # CRITICAL: Strip BOS/decoder_start_token_id from labels if present.
    # The model adds this internally during training. Without stripping,
    # there's a systematic one-token shift in the loss.
    # Ref: HuggingFace Whisper fine-tuning guide
    decoder_start_token_id = processor.tokenizer.convert_tokens_to_ids("<|startoftranscript|>")
    if decoder_start_token_id is not None and (label_ids[:, 0] == decoder_start_token_id).all():
        label_ids = label_ids[:, 1:]

    # Replace padding token id with -100 so it is ignored by loss
    label_ids = label_ids.masked_fill(
        label_ids == processor.tokenizer.pad_token_id, -100
    )
...
def evaluate(model, dataloader, processor, device):
    """Run greedy decoding on a dataset and compute CER/WER."""
    model.eval()
    all_refs = []
    all_hyps = []
    all_ids = []
...
def train(config: dict, data_dir: str, output_dir: str, seed: int, dry_run: bool):
    ...
    # Configure for Adja fine-tuning
    model.config.forced_decoder_ids = None
    model.generation_config.forced_decoder_ids = None
    model.generation_config.task = "transcribe"
    model.generation_config.language = None  # Adja not in Whisper vocab
    ...
    train_dataset = AdjaWhisperDataset(str(train_manifest), processor, max_audio_sec)
    dev_dataset = AdjaWhisperDataset(str(dev_manifest), processor, max_audio_sec)
    ...
    for epoch in range(max_epochs):
        ...
        eval_results = evaluate(model, dev_loader, processor, device)
    ...
    ckpt_dir = output_path / "best_checkpoint"
    if ckpt_dir.exists():
        from transformers import WhisperForConditionalGeneration as WFC

        model = WFC.from_pretrained(str(ckpt_dir))
        model.to(device)

    final_eval = evaluate(model, dev_loader, processor, device)

    # ---- Save decode samples ----
    samples_path = output_path / "decode_samples.txt"
    n_samples = min(num_decode_samples, len(final_eval["references"]))
    with open(samples_path, "w", encoding="utf-8") as f:
        f.write(f"C2 Whisper Fine-Tune -- Decode Samples (best checkpoint)\n")
        f.write(f"{'='*70}\n\n")
    ...
    # ---- Save metrics ----
    metrics = {
        "experiment": "C2_whisper_finetune",
        "model_name": config["model_name"],
...
# ===========================================================================
def main():
    parser = argparse.ArgumentParser(description="C2: Fine-tune Whisper on Adja ASR")
    ...
    print(f"C2: Whisper Fine-Tune on Adja ASR")
