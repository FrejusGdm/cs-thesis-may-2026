...
def train(config: dict, data_dir: str, output_dir: str, seed: int, dry_run: bool):
    ...
    # Load adapter from config — E1 uses Ewe ("ewe"), the closest Gbe
    # relative to Adja. Compare against C3 which uses French ("fra").
    adapter_lang = config.get("target_lang", "ewe")
    print(f"Loading adapter: {adapter_lang}")
    model = Wav2Vec2ForCTC.from_pretrained(
        model_name,
        target_lang=adapter_lang,
